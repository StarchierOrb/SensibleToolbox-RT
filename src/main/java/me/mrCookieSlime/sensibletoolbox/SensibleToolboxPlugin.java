...
import me.mrCookieSlime.sensibletoolbox.items.energycells.OneKEnergyCell;
import me.mrCookieSlime.sensibletoolbox.items.energycells.FiveKEnergyCell;
import me.mrCookieSlime.sensibletoolbox.items.energycells.TwKEnergyCell;
...
public class SensibleToolboxPlugin extends JavaPlugin implements ConfigurationListener {
    ...
    private ResidenceApi Res = null;
    private PlotAPI plot = null;
...
// test end
    ...
    public void registerItems() {
        ...
        itemRegistry.registerItem(new OneKEnergyCell(), this, CONFIG_NODE, PERMISSION_NODE);
        itemRegistry.registerItem(new FiveKEnergyCell(), this, CONFIG_NODE, PERMISSION_NODE);
        itemRegistry.registerItem(new TwKEnergyCell(), this, CONFIG_NODE, PERMISSION_NODE);
    ...
    private void setupWorldGuard() {
        ...
        if (plugin != null && plugin.isEnabled() && plugin instanceof WorldGuardPlugin) {
			...
			getLogger().log(Level.INFO, "找到 WorldGuard，成功挂载！ 插件版本：" + plugin.getDescription().getVersion());
    ...
    private void setupPreciousStones() {
        ...
        if (plugin != null && plugin.isEnabled() && plugin instanceof PreciousStones) {
			...
			getLogger().log(Level.INFO, "找到 PreciousStones，成功挂载！ 插件版本：" + plugin.getDescription().getVersion());
    ...
    private void setupProtocolLib() {
        ...
        if (Bukkit.getPluginManager().getPlugin("ProtocolLib")!=null) {
			...
			getLogger().log(Level.INFO, "找到 ProtocolLib，成功挂载！ 插件版本：" + pLib.getDescription().getVersion());
        }
        if (protocolLibEnabled) {
            if (getConfig().getBoolean("options.glowing_items"))ItemGlow.init(this);
        } 
        else {
            LogUtils.warning("ProtocolLib 没能成功挂载 - 是不是没有安装该插件呢:");
            LogUtils.warning("  发光效果, 附加的粒子效果将被禁用, 消音器将不会被注册.");
        }
    }
    ...
    private void setupMultiverse() {
        ...
        if (mvPlugin != null && mvPlugin.isEnabled() && mvPlugin instanceof MultiverseCore) {
			...
			getLogger().log(Level.INFO, "找到 Multiverse-Core，成功挂载！ 插件版本：" + mvPlugin.getDescription().getVersion());
        }
    }
    ...
    private void setupLWC() {
        ...
        if (lwcPlugin != null && lwcPlugin.isEnabled() && lwcPlugin instanceof LWCPlugin) {
			...
			getLogger().log(Level.INFO, "找到 LWC，成功挂载！ LWC版本：" + lwcPlugin.getDescription().getVersion());
        }
    }
    private void setupRes() {
        Plugin resPlugin = getServer().getPluginManager().getPlugin("Residence");
        if (resPlugin != null && resPlugin.isEnabled() && resPlugin instanceof Residence) {
            Res = ((Residence) resPlugin).getAPI();
            Debugger.getInstance().debug("Hooked Res v" + resPlugin.getDescription().getVersion());
			getLogger().log(Level.INFO, "找到 Residence，成功挂载！ 插件版本：" + resPlugin.getDescription().getVersion());
        }
    }
    private void setupPlot() {
        Plugin plotPlugin = getServer().getPluginManager().getPlugin("PlotSquared");
        if (plotPlugin != null && plotPlugin.isEnabled() && plotPlugin instanceof PlotAPI) {
            plot = ((PlotAPI) plotPlugin);
            Debugger.getInstance().debug("Hooked PlotSquared v" + plotPlugin.getDescription().getVersion());
    ...
    public ResidenceApi getRes() 				            {			return Res;															        }
    public PlotAPI getplot() 				                {			return plot;															    }
//test
