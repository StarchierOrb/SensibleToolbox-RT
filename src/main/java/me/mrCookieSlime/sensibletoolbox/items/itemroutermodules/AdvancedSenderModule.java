...
public class AdvancedSenderModule extends DirectionalItemRouterModule {
    ...
    public String[] getLore() {
        return new String[]{
				"TIDX:01",
                "可以将此模块放入物品路由器中来激活",
                "可以远程发射物品到一个已经连接的接收器模块",
                " 已经连接的接收器模块 ",
				"工作半径" + RANGE + "-方块 半径",
                " (工作时不需要玩家在此区块)",
                "左键点击 已经安装接收器模块",
                "         的物品路由器: " + ChatColor.RESET + " 连接传送器",
                UnicodeSymbol.ARROW_UP.toUnicode() + " + 左键点击: " + ChatColor.RESET + " 断开传送器的连接"
        };
    }
    ...
    private int sendItems(ReceiverModule receiver) {
        Debugger.getInstance().debug(this.getItemRouter() + ": 传送器正在传送物品到接收器模块： " + receiver.getItemRouter());
