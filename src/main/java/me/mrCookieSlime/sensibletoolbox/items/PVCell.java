...
public class PVCell extends BaseSTBItem {
    private static final MaterialData md = new MaterialData(Material.GOLD_HELMET);
...
//    private static final int MAX_LIFESPAN = 2000; // 100 real seconds (testing)
    ...
    public String[] getLore() {
        return new String[] { "TID:630", "光伏电池", "可以将此物品插入太阳能发电机", "右键点击太阳能发电机: 直接插入"};
    }
    ...
    public static String formatCellLife(int lifespan) {
        int sec = lifespan / 20;
        if (sec >= 60) {
            return ChatColor.RESET + "使用寿命: " + ChatColor.YELLOW.toString() + (sec / 60) + " 分钟";
        } else {
            return ChatColor.RESET + "使用寿命: " + ChatColor.YELLOW.toString() + sec + " 秒";
        }
    }
}
