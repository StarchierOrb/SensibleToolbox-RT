...
public class EnderTuner extends BaseSTBItem {
    ...
    private static final ItemStack GLOBAL_TEXTURE = GUIUtil.makeTexture(
            STBUtil.makeColouredMaterial(Material.STAINED_GLASS, DyeColor.BLUE), "全球次元存储", "给所有玩家开放的", "公共仓库");
    private static final ItemStack PERSONAL_TEXTURE = GUIUtil.makeTexture(
            STBUtil.makeColouredMaterial(Material.STAINED_GLASS, DyeColor.YELLOW), "私人次元存储", "为单独的玩家", "提供私人仓库");
    ...
    public String[] getLore() {
        return new String[] {
				"TID:99",
                "通过高频振动建立六个维度的连接",
                "右键点击 末影同步网络: " + ChatColor.RESET + "更改仓库维度",
                "右键点击: " + ChatColor.RESET + "打开调谐界面"
        };
    }
    ...
    private InventoryGUI makeTuningGUI(Player player) {
        final InventoryGUI gui = GUIUtil.createGUI(player, this, TUNING_GUI_SIZE, ChatColor.DARK_PURPLE + "末影调谐器");
        ...
        if (tuningBlock != null) {
            ...
            gui.addLabel("末影同步网络", TUNED_ITEM_SLOT - 1, null);
        ...
        else gui.addLabel("末影背包", TUNED_ITEM_SLOT - 1, null, "将末影背包或者", "末影同步网络放入这里", "来调谐末影维度频率");
        ...
        gui.addGadget(new NumericGadget(gui, FREQUENCY_BUTTON_SLOT, "末影维度频率", new IntRange(1, EnderStorage.MAX_ENDER_FREQUENCY), freq, 1, 10, new NumericGadget.NumericListener() {
            @Override
