...
public class PaintCan extends BaseSTBBlock implements LevelMonitor.LevelReporter {
    ...
    public InventoryGUI createGUI() {
        ...
        gui.addLabel("混合成分", 0, null, "混合油漆步骤:", "· 放入牛奶桶 & 染料", "为其他物品染色:", "· 放入可以染色的物品");
        for (int slot : ITEM_SLOTS) {
            gui.setSlotType(slot, InventoryGUI.SlotType.ITEM);
        }
        String[] lore = new String[] { "把牛奶和染料混合制成颜料", "或者为可以染色的物品着色", "* 将使用现存的颜料"};
        gui.addGadget(new ButtonGadget(gui, 12, "混合或染色", lore, MIX_TEXTURE, new Runnable() {
            @Override
            public void run() {
                if (tryMix()) {
        ...
        lore = new String[] { "警告：这将清空油漆罐", "，不能撤消!" };
        gui.addGadget(new ButtonGadget(gui, 13, ChatColor.RED.toString() + ChatColor.UNDERLINE + UnicodeSymbol.KILL.toUnicode() +" 清空颜料 "+ UnicodeSymbol.KILL.toUnicode(), lore, EMPTY_TEXTURE, new Runnable() {
            @Override
            public void run() {
                emptyPaintCan();
            }
    ...
    public String getLevelMessage() {
        ...
        return ChatColor.WHITE + "颜料等级: " + getPaintLevel() + "/" + MAX_PAINT_LEVEL + " " + cc + getColour();
    }
    ...
    protected String[] getSignLabel(BlockFace face) {
        ...
        res[3] = cc + StringUtils.repeat(UnicodeSymbol.WAY.toUnicode(), (getPaintLevel() * 13) / getMaxPaintLevel());
        return res;
    }
