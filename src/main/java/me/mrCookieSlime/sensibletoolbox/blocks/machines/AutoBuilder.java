...
public class AutoBuilder extends BaseSTBMachine {
    ...
    public String[] getLore() {
        return new String[] { "使用该机器可以进行建造或者清空", "某一个区域. ", "* 需要使用GPS坐标定位器来锁定位置"};
    }
    ...
    public InventoryGUI createGUI() {
        ...
        gui.addGadget(new ButtonGadget(gui, START_BUTTON_SLOT, "开始", null, null, new Runnable() {
            @Override
            public void run() {
        ...
        gui.addLabel(ChatColor.WHITE + "目前状态: " + c + status, STATUS_SLOT, status.makeTexture(), status.getText());

        gui.addLabel("建造背包", 29, null);
    ...
    private void setStatus(BuilderStatus status) {
        if (status != this.status) {
            this.status = status;
            ChatColor c = STBUtil.dyeColorToChatColor(status.getColor());
            getGUI().addLabel(ChatColor.WHITE + "目前状态: " + c + status, STATUS_SLOT, status.makeTexture(), status.getText());
    ...
    private void setupLandMarkerLabel(InventoryGUI gui, Location loc1, Location loc2) {
        if (workArea == null) {
            gui.addLabel("GPS坐标定位器", 11, null, "在这里放置GPS坐标定位器", "来锁定目标区域", "* 需要选择两个位置", "  才能正常工作");
        } else {
            int v = workArea.volume();
            String s = v == 1 ? "" : "s";
            gui.addLabel("GPS坐标定位器", 11, null, "已锁定区域:",
                    MiscUtil.formatLocation(loc1), MiscUtil.formatLocation(loc2), v + " 方块" + s);
        }
    }
    ...
    public int onShiftClickInsert(HumanEntity player, int slot, ItemStack toInsert) {
        ...
        if (item instanceof LandMarker && getStatus() != BuilderStatus.RUNNING) {
            if (((LandMarker) item).getMarkedLocation() != null) {
                ...
                STBUtil.complain((Player) player, "GPS坐标定位器 还没有获取到坐标！!");
            }
    ...
    protected String[] getSignLabel(BlockFace face) {
        ...
        label[2] = "-( " + STBUtil.dyeColorToChatColor(getStatus().getColor()) + UnicodeSymbol.WAY.toUnicode() + ChatColor.RESET + " )-";
        return label;
    }
    ...
    public enum BuilderStatus {
        READY(DyeColor.LIME, "设备准备就绪，可以工作!"),
        NO_WORKAREA(DyeColor.YELLOW, "还没有锁定", "工作区域"),
        NO_INVENTORY(DyeColor.RED, "建筑材料已耗尽!", "请放置一些方块", "到建筑背包！", "然后点击开始即可工作"),
        NO_PERMISSION(DyeColor.RED, "你没有相应的权限", "在所选定的区域中", "进行建造"),
        TOO_NEAR(DyeColor.RED, "机器被放在了选定区域里！", "机器需要在选定区域外才能工作!"),
        TOO_FAR(DyeColor.RED, "机器距离选定区域太远了", "没办法开工!", "请将机器放在距离选定区域" + MAX_DISTANCE + " 个方块内的地方"),
        LM_WORLDS_DIFFERENT(DyeColor.RED, "GPS坐标定位器", "所定位的区域在不同的世界!"),
        RUNNING(DyeColor.LIGHT_BLUE, "机器运行中！", "点击开始按钮暂停"),
        PAUSED(DyeColor.ORANGE, "机器已经暂停运行", "点击开始按钮开始工作"),
        FINISHED(DyeColor.WHITE, "建造完成!", "正在等待下一次建造...");
